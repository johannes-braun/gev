...
namespace gev::game
{
  tonemap::tonemap()
  {
    ...
    auto const shader = gev::create_shader(gev::load_spv(gev_game_shaders::shaders::tonemap_comp));
  ...
  void tonemap::apply(vk::CommandBuffer c, float gamma, render_target_2d const& src, render_target_2d const& dst)
  {
    ...
    c.pushConstants<float>(_layout.get(), vk::ShaderStageFlagBits::eCompute, 0, gamma);
