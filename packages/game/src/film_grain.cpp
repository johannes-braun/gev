...
namespace gev::game
{
  film_grain::film_grain()
  {
    ...
    options_range.size = 2*sizeof(float);
    ...
    auto const shader = gev::create_shader(gev::load_spv(gev_game_shaders::shaders::film_grain_comp));
  ...
  void film_grain::apply(vk::CommandBuffer c, float strength, render_target_2d const& src, render_target_2d const& dst)
  {
    ...
    struct
    {
      float strength;
      float clock;
    } options{.strength = strength, .clock = float(glfwGetTime())};
